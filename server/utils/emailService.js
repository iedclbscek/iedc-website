...
const sendEmailWithRateLimit = async (mailOptions, retryCount = 0) => {
  ...
  try {
    ...
    // No SendGrid fallback anymore – we rely on configured SMTP/Gmail
...
const queueEmail = (mailOptions) => {
  ...
  const enforcedMailOptions = {
    ...mailOptions,
    from: `"IEDC LBSCEK" <${verifiedFrom}>`,
  };

  if (mailOptions.from !== enforcedMailOptions.from) {
    console.log(
      `✉️ Normalized FROM address for compliance: ${mailOptions.from} -> ${enforcedMailOptions.from}`
    );
  }

  return new Promise((resolve) => {
    emailQueue.push({ mailOptions: enforcedMailOptions, resolve });
    processEmailQueue();
  });
};
...
export const sendInvitationEmail = async (email, name, resetToken) => {
  ...
  const mailOptions = {
    from: `"IEDC LBSCEK" <${getVerifiedFromAddress()}>`,
    to: email,
...
// Simple health-check across available providers used by routes/auth.js
export const testEmailService = async () => {
  ...
  try {
    const sg = await testSendGridAuth();
    status.sendgrid = !!sg.valid;
  } catch (_) {}

  // Try generic SMTP (or Gmail via STARTTLS) if creds present
  ...
  // Check gmail fallback creds specifically
  if (process.env.EMAIL_USER && process.env.EMAIL_PASS) {
    try {
      const gt = nodemailer.createTransport({
        service: "gmail",
        auth: { user: process.env.EMAIL_USER, pass: process.env.EMAIL_PASS },
      });
      await gt.verify();
      status.gmailFallback = true;
    } catch (_) {}
  }
